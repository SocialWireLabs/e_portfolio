...
$options = array('type_subtype_pairs' => array('object' => 'e_portfolio_artifact'), 'limit' => false, 'count' => true, 'container_guid' => $e_portfolio_page_guid);
$count_artifacts = elgg_get_entities_from_metadata($options);
$artifact_number = $count_artifacts + 1;
...
elgg_make_sticky_form('add_artifact_e_portfolio');
   
// Make sure the title is not blank
if (empty($title)) {
   register_error(elgg_echo("e_portfolio:artifact_title_blank"));
   forward(elgg_get_site_url() . 'e_portfolio/add_artifact/' . $e_portfolio_page_guid . '/?artifact_type=' . $artifact_type);
}
...
if ((strcmp($artifact_type,"urls_files")==0)&&((($file_counter+$number_urls)==0)||((($number_urls)==0)&&($_FILES['upload']['name'][0] == "")))){
   register_error(elgg_echo('e_portfolio:not_artifact_files'));
   forward(elgg_get_site_url() . 'e_portfolio/add_artifact/' . $e_portfolio_page_guid . '/?artifact_type=' . $artifact_type);
}  
...
$e_portfolio_artifact = new ElggObject();
$e_portfolio_artifact->subtype = "e_portfolio_artifact";
$e_portfolio_artifact->owner_guid = $user_guid;
$e_portfolio_artifact->container_guid = $e_portfolio_page_guid;
...
$e_portfolio_artifact->artifact_number = $artifact_number;
...
$e_portfolio_artifact->artifact_type = $artifact_type;
switch ($artifact_type) {
   case 'urls_files':
   ...
   case 'video':
      ...
      add_entity_relationship($e_portfolio_artifact->getGUID(),'e_portfolio_artifact_file_link',$izap_videos->getGUID());
      break;
}
if (($file_counter>0)&&($_FILES['upload']['name'][0] != "")){
   for($i=0; $i<$file_counter; $i++){
      add_entity_relationship($e_portfolio_artifact->getGUID(),'e_portfolio_artifact_file_link',$file[$i]->getGUID());
   }
}
...
// Success message
system_message(elgg_echo("e_portfolio:artifact_created"));
