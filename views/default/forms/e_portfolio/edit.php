...
$e_portfoliopost=$vars['entity']->getGUID();
$e_portfolio = get_entity($e_portfoliopost);
$action = "e_portfolio/edit";
...
if (!elgg_is_sticky_form('edit_e_portfolio')) {
   $description = $e_portfolio->description;
   if (strcmp($e_portfolio->allow_comments,'on')==0){  
      $allow_comments = true;
   } else {
      $allow_comments = false;
   }
   $tags = $e_portfolio->tags;
   $access_id = $e_portfolio->access_id;
} else {
  $description = elgg_get_sticky_value('edit_e_portfolio','description');
  $allow_comments = elgg_get_sticky_value('edit_e_portfolio','allow_comments');
  $tags = elgg_get_sticky_value('edit_e_portfolio','e_portfoliotags');
  $access_id = elgg_get_sticky_value('edit_e_portfolio','access_id');
}
...
<form action="<?php echo elgg_get_site_url()."action/".$action?>" name="edit_e_portfolio" enctype="multipart/form-data" method="post">
